							  ...
							  double *distance)
{
	...
	dot = (double) rawDot;
	scale = TQ_GRAPH_CODEBOOK_SCALE * TQ_GRAPH_CODEBOOK_SCALE *
		(double) sampleDims;
	if (mode == TQ_SCORE_IP)
							  ...
							  double *distance)
{
	...
	if ((mode != TQ_SCORE_L2 && mode != TQ_SCORE_COSINE && mode != TQ_SCORE_IP) ||
		state->dimensions < 1024 || state->tqBits != 2)
		return false;
	...
	dot = (double) rawDot;
	scale = TQ_GRAPH_CODEBOOK2_SCALE * TQ_GRAPH_CODEBOOK2_SCALE *
		(double) sampleDims;
	if (mode == TQ_SCORE_IP)
		*distance = -((double) aNode->scale * (double) bNode->scale *
					  dot / scale);
