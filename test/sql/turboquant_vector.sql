...
CREATE TABLE tq_default_dims (id int, val vector(3072));
INSERT INTO tq_default_dims
SELECT i, ARRAY(
	SELECT sin(0.001 * (g + i * 17))::real
	FROM generate_series(1, 3072) g
)::vector(3072)
FROM generate_series(1, 20) i;
CREATE INDEX tq_default_dims_cos_idx ON tq_default_dims
	USING turboquant (val vector_cosine_ops) WITH (routing = graph);
DO $$
DECLARE
	query vector(3072);
	row_count int;
	stats jsonb;
BEGIN
	IF tq_index_stats('tq_default_dims_cos_idx'::regclass)->>'tq_plus' <> 'true' THEN
		RAISE EXCEPTION 'expected tq_plus metadata for 3072-dimensional cosine index';
	END IF;
	SELECT ARRAY(
		SELECT sin(0.001 * (g + 999))::real
		FROM generate_series(1, 3072) g
	)::vector(3072) INTO query;
	SELECT count(*) INTO row_count
	FROM (SELECT id FROM tq_default_dims ORDER BY val <=> query LIMIT 5) s;
	IF row_count <> 5 THEN
		RAISE EXCEPTION 'expected 3072-dimensional cosine graph rows';
	END IF;
	stats := tq_last_scan_stats();
	IF stats->>'scan_orchestration' <> 'graph_native' THEN
		RAISE EXCEPTION 'expected native graph scan for 3072-dimensional cosine index';
	END IF;
END
$$;
DROP TABLE tq_default_dims;
...
DECLARE
	...
	live_nodes int;
	dead_nodes int;
	dead_neighbor_refs int;
BEGIN
	stats := tq_index_stats('tq_vacuum_chain_idx'::regclass);
	vacuum_pages := (stats -> 'graph_page_last_op_counts' ->> 'vacuum_delete')::int;
	repair_pages := (stats -> 'graph_page_last_op_counts' ->> 'vacuum_repair')::int;
	live_nodes := (stats ->> 'tq_live_node_count')::int;
	dead_nodes := (stats ->> 'tq_dead_node_count')::int;
	dead_neighbor_refs := (stats ->> 'graph_dead_neighbor_refs')::int;

	IF vacuum_pages < 2 THEN
		RAISE EXCEPTION 'expected vacuum to follow linked TurboQuant code pages, got % pages',
			vacuum_pages;
	END IF;
	IF repair_pages < 1 THEN
		RAISE EXCEPTION 'expected vacuum to repair TurboQuant adjacency pages';
	END IF;
	IF live_nodes <> 10 OR dead_nodes <> 30 THEN
		RAISE EXCEPTION 'expected 10 live and 30 dead TurboQuant nodes, got live %, dead %',
			live_nodes, dead_nodes;
	END IF;
	IF dead_neighbor_refs <> 0 THEN
		RAISE EXCEPTION 'expected vacuum to remove dead TurboQuant adjacency refs, got %',
			dead_neighbor_refs;
	END IF;
END
$$;
